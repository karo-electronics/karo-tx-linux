			...
			size_t block_size)
{
	...
	csum_le = cpu_to_le32(dm_block_csum_data(&disk_header->not_used, block_size - sizeof(__le32)));
	if (csum_le != disk_header->csum) {
		DMERR("bitmap check failed csum %u wanted %u",
		      le32_to_cpu(csum_le), le32_to_cpu(disk_header->csum));
		return -EILSEQ;
	}
