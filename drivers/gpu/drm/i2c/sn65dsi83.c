...
static int sn65dsi83_pll_init(struct sn65dsi83 *dsi83)
{
	...
	dev_dbg(&dsi83->client->dev, "%s@%d: lvds_clk=%lu mipi_dsi_clk=%lu lvds_clk_src=%u ref_clk_mult=%u dsi_clk_div=%u\n",
		__func__, __LINE__, lvds_clk, mipi_dsi_clk, lvds_clk_src, ref_clk_mult, dsi_clk_div);
...
static int sn65dsi83_of_get_bus_format(struct device *dev, struct device_node *np)
{
	...
	of_property_read_u32(np, "lvds-data-width", &datawidth);
